...
CBPlusTree::CBPlusTree(const CCollection& collection, CPage& pge, CDiskMan& cdm, size_t pageSize)
    : m_root{0, 0},
        m_pageSize(pageSize),
        m_page(pge),
        m_diskman(cdm),
    ...
    CColumn* pkCol = m_collection.m_cols[m_collection.m_pkPos];
    ...
    m_nodeSize = hdrSize;
    size_t remainSize = 0;
    remainSize = m_pageSize * dpfs_lba_size - hdrSize;

    // key count = (remain size - one child)/(key size + child size (with one extra child))
    size_t keyCnt = (remainSize - sizeof(uint64_t)) / (keyLen + sizeof(uint64_t) /*keysize + childsize*/);
    ...
    // reserve 1 for split
    m_indexOrder = static_cast<size_t>(keyCnt) - 1;
    m_nodeSize += (m_indexOrder * keyLen) + ((m_indexOrder + 1) * sizeof(uint64_t));

    for(int i = 0; i < m_collection.m_cols.size(); ++i) {
        CColumn* col = m_collection.m_cols[i];
    ...
    m_rowOrder = ((m_pageSize * dpfs_lba_size - hdrSize) / (keyLen + m_rowLen)) - 1;
...
int CBPlusTree::insert(const KEY_T& key, const void* row, uint32_t len) {
    ...
    if (rc == SPLIT) {
        ...
        if(high == 1) {
            ...
            CBPlusTree::NodeData newRoot(m_page);
            rc = newRoot.initNode(false, m_indexOrder);
            if(rc != 0) {
                return rc;
            }
        ...
        // split root node
        NodeData newRoot(m_page);
        
        // right.initNode(false, m_indexOrder);
        newRoot.initNode(false, m_indexOrder);
        ...
        // right.nodeData->hdr->childIsLeaf = root.nodeData->hdr->childIsLeaf;

        // newRoot.nodeData->hdr->parent = 0;
        // root.nodeData->hdr->parent = newRoot.self.bid;
        // right.nodeData->hdr->parent = newRoot.self.bid;
        ...
        // update parent links
        // update_parent(m_root, root.self.bid);
        // update_parent(upChild, root.self.bid);
        ++high;
        return 0;
    }
    return 0;
...
int32_t CBPlusTree::remove_recursive(const bidx& idx, const KEY_T& key, KEY_T& upKey, bidx& upChild, bool isLeaf) {
    ...
    rc = remove_recursive(nextNode, key, childUp, childNew, node.nodeData->hdr->childIsLeaf ? true : false);
...
int32_t CBPlusTree::combine_child(NodeData& node, int32_t idxChild, KEY_T& childUp, bidx& childNew) {
    ...
    NodeData left(m_page), right(m_page), target(m_page);
    int rc = 0;
    // load left child
    bidx idx = {nodeId, 0}; 
    ...
    if(borrowNode->size() > (isLeaf ? m_rowOrder : m_indexOrder) / 2) {
        ct = BORROW;
    } else {
        ct = MERGE;
    }
...
RIGHT
    ...
    */
    ...
    if(isLeaf) {
        ...
        KEY_T* fromKey = nullptr;
        int rc = fromNode.keyVec->at(fromKeyIdx, fromKey);
        if(rc != 0) return rc;

        KEY_T* parentKey = nullptr;
        ...
        *parentKey = *fromKey;
        ...
        if(fromLeft) {
            // insert to left
            rc = toNode.pushFrontChild(targetKey, fromNodeChild);
            if(rc != 0) return rc;
            fromNode.popBackChild();
        } else {

            // insert to right
            rc = toNode.pushBackChild(targetKey, fromNodeChild);
            if(rc != 0) return rc;
...
int CBPlusTree::NodeData::initNode(bool isLeaf, int32_t order) {
    ...
    void* zptr = isLeaf ? m_page.alloczptr(ROWPAGESIZE) : m_page.alloczptr(PAGESIZE);
    if(zptr == nullptr) {
        return -ENOSPC;
    }

    nodeData = new nd(isLeaf, zptr);
    memset(nodeData->data, 0, isLeaf ? ROWPAGESIZE : PAGESIZE);
...
int CBPlusTree::NodeData::initNodeByLoad(bool isLeaf, int32_t order, void* zptr) {
...
int CBPlusTree::NodeData::insertChild(const KEY_T& key, uint64_t lchild, uint64_t rchild) {
    ...
    /*           0 1  2  3
            ...
            [-1,0,1]
    */
    if(lchild) childVec->at(pos) = lchild;
    
    rc = childVec->insert(pos + 1, rchild);
    if (rc < 0) {
...
int CBPlusTree::NodeData::assign(const NodeData& target, int begin, int end) noexcept {
    int rc = keyVec->assign(&target.keys[begin], &target.keys[end]);
    if (rc < 0) {
        return rc;
    }
    if (!nodeData->hdr->leaf) {
        ...
        rc = rowVec->assign(&target.rowVec->row[begin * ROWLEN], &target.rowVec->row[end * ROWLEN]);
        if (rc < 0) {
            return rc;
        }
    }
