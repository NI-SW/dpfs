...
std::list<ctrlr_entry*> g_controllers;
std::list<ns_entry*> g_namespaces;
...
cleanup(void)
{
    ...
    for(auto it = g_namespaces.begin(); it != g_namespaces.end();) {
        free(*it);
        it = g_namespaces.erase(it);
        continue;
        ++it;
    }
